...
Write-Output "----------------------------------------"
Write-Output "  RAPPORT QUOTIDIEN — $HostName (Windows)"
Write-Output "  $Today"
Write-Output "----------------------------------------"
Write-Output ""
Write-Output "[i] Instantane au moment de l'execution (Windows n'a pas d'historique sar)."
Write-Output ""
...
Write-Output ("up {0}j {1}h {2}min (depuis {3:yyyy-MM-dd HH:mm})" -f $uptime.Days, $uptime.Hours, $uptime.Minutes, $boot)
Write-Output ""

# -- CPU --
Write-Output "-- CPU — $nCpu coeurs logiques ($($cpuInfo.Name.Trim())) --"
...
Write-Output ("> Instantane — Used: {0}% | Idle: {1}%" -f $cpuUse, $cpuIdle)
Write-Output ""
...
Write-Output ("> Utilisation : {0}%  ({1} MB used | {2} MB dispo)" -f $usedPct, $usedMb, $freeMb)
...
Get-CimInstance Win32_LogicalDisk -Filter "DriveType=3" | ForEach-Object {
    ...
    Write-Output ("> {0} — Taille: {1} Go | Used: {2} Go ({3}%) | Free: {4} Go" -f $_.DeviceID, $sizeGb, $usedGb, $pct, $freeGb)
}
Write-Output ""
...
$procs = Get-Process
Write-Output ("> Total : {0} processus" -f $procs.Count)
