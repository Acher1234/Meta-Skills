...
def format_meeting_markdown(meeting: dict, summary: str = None, transcript: str = None) -> str:
    ...
    # default_summary can be null in the API response.
    if not summary:
        default_summary = meeting.get('default_summary') or {}
        if isinstance(default_summary, dict):
            summary = default_summary.get('markdown', '') or ''
        else:
    ...
    if not transcript:
        ...
        if isinstance(transcript_data, list):
            ...
            for entry in transcript_data:
                speaker = (entry.get('speaker') or {}).get('name') or 'Unknown'
                text = entry.get('text', '')
