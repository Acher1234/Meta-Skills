...
> **Prerequisite — load credentials first**
>
> Before using this sub-skill, load the workspace `.env` via the meta-jira CLI:
>
> ```bash
> cd ~/.meta-skills/skills/productivity/meta-jira/scripts
> python cli.py env
> # or verify: python cli.py env-check
> ```
>
> Requires `CURRENT_SKILL_DIRECTORY` pointing at the registered meta-jira skill dir (where `.env` lives).
