...
ZIA URL cloud applications and URL categories (read-only) via `LegacyZIAClient`.
Implemented in `scripts/zia/url_cloud_apps.py`.
...
| `/zscaler_zia_url_cloud_apps_categories` | `python scripts/cli.py zia url-cloud-apps categories [--search {SEARCH}]` | List URL categories (lite) |
| `/zscaler_zia_url_cloud_apps_category` | `python scripts/cli.py zia url-cloud-apps category [--id {CATEGORY_ID}] [--name {CATEGORY_NAME}]` | Get a URL category (lite) |
