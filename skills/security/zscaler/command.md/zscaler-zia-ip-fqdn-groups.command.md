...
Confirm before `update`. Unspecified fields are kept. `--address` replaces members unless `--append`.
...
`--exclude-type` / `--type`: `DSTN_IP`, `DSTN_FQDN`, `DSTN_DOMAIN`, `DSTN_OTHER`. Repeatable: `--address`, `--ip-category`, `--country`. Rename: `--new-name`. Optional client overrides: `--username` `--password` `--api-key` `--cloud`.
