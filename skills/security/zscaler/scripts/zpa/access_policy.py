...
class AccessPolicyClient(ZpaClient):
    @staticmethod
    ...
    def list_rules(self, search: str | None = None) -> list[dict[str, Any]]:
        ...
        with self.get_client() as client:
            ...
            while True:
                ...
                if err:
                    raise RuntimeError(f"Failed to list access policy rules: {err}")
        ...
        *,
        ...
        if rule_id is not None and str(rule_id).strip():
            ...
            with self.get_client() as client:
                ...
                if err:
                    raise RuntimeError(f"Failed to get access policy rule {rid}: {err}")
                if rule is None:
                    raise RuntimeError(f"Access policy rule not found: {rid}")
        ...
        if not matches:
            raise RuntimeError(f"Access policy rule not found: {rule_name!r}")
        if len(matches) > 1:
            ids = ", ".join(str(rule.get("id")) for rule in matches)
            raise RuntimeError(
                f"multiple access policy rules named {rule_name!r}: {ids}"
            )
        return matches[0]
    ...
    def register(sub: argparse._SubParsersAction) -> None:
        ...
        p = sub.add_parser("access-policy", help="ZPA access policy")
        cmds = p.add_subparsers(required=True)

        u_list = cmds.add_parser("list", help="List access policy rules")
        u_list.add_argument("--search", default="", help="Filter by name")
        u_list.set_defaults(func=client.cmd_list)

        u_get = cmds.add_parser("get", help="Get an access policy rule")
