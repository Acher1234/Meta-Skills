...
Agent workflow:
  ...
  3. Run --auth-url. Starts local HTTP redirect server, prints OAuth URL for the user.
  4. User opens URL, authorizes, lands on localhost page with the code to copy.
  5. User pastes the code. Agent runs --auth-code CODE (stops the HTTP server).
...
PENDING_AUTH_PATH = SkillCred(_SKILL, ["google_oauth_pending.json"])
HTTP_LOCAL = Path(__file__).resolve().parent / "http-local.py"
...
def _load_token_payload(path: Path | SkillCred = TOKEN_PATH) -> dict:
    try:
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json next to SKILL.md."""
...
def get_auth_url():
    ...
    if PENDING_AUTH_PATH.exists():
        try:
            old = json.loads(PENDING_AUTH_PATH.read_text())
            _stop_oauth_http_server(old.get("http_server_pid"))
        except Exception:
            pass
...
def exchange_auth_code(code: str):
    ...
    http_server_pid = pending_auth.get("http_server_pid")
    try:
        raw_callback = code
        ...
        print(f"Skill-scoped token location: {display_path(TOKEN_PATH)}")
    finally:
        _stop_oauth_http_server(http_server_pid)
...
def revoke():
    ...
    if PENDING_AUTH_PATH.exists():
        try:
            ...
            _stop_oauth_http_server(pending.get("http_server_pid"))
        except Exception:
            pass
    PENDING_AUTH_PATH.unlink(missing_ok=True)
