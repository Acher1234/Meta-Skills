...
Trim extra background around a logo with **Pillow**. Accepts WebP, PNG, JPG, and **SVG / SVGZ** (rasterized first). Implemented in `scripts/trim.py` (`Trim`).
...
Placeholders: `{INPUT}`, `{OUTPUT}`, `{PADDING}`. Optional: `--tolerance {TOLERANCE}` (default 24), `--force`, `--svg-dpi`, `--svg-width`, `--svg-scale`. Default output: `{stem}-trim.png`.
